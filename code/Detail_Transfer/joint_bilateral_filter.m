% img_for_weight : this image will be used to calculate intensity gaussian filter

% spatial_sigma : standard deviation of gaussian for difference of spatial location

% intensity-sigma : standard deviation of gaussian for difference of pixel intensity

% filter_size : odd value is preferable

function [output_img] = joint_bilateral_filter(input_img, img_for_weight, spatial_sigma, intensity_sigma, filter_size)
    [row, col, channel] = size(img_for_weight);
    ...
    img_for_weight = padarray(img_for_weight, [filter_size filter_size], -1);
    ...
    for i = (1+filter_size : filter_size+row)
        for j = (1+filter_size : filter_size+col)
            ...
            mask = img_for_weight(i-filter_size:i+filter_size, j-filter_size:j+filter_size, :);
            mask = logical(mask ~= -1);
            
            % constructing intensity gaussian filter
            intensity_gauss_filter = img_for_weight(i-filter_size:i+filter_size, j-filter_size:j+filter_size, :);
            
            red_component = img_for_weight(i, j, 1);
            green_component = img_for_weight(i, j, 2);
            blue_component = img_for_weight(i, j, 3);
